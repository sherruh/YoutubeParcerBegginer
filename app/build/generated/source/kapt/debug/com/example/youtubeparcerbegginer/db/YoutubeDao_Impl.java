...
public final class YoutubeDao_Impl implements YoutubeDao {
  ...
  private final EntityInsertionAdapter<DetailVideoModel> __insertionAdapterOfDetailVideoModel;
  ...
  public YoutubeDao_Impl(RoomDatabase __db) {
    ...
    this.__insertionAdapterOfDetailVideoModel = new EntityInsertionAdapter<DetailVideoModel>(__db) {
      @Override
      public String createQuery() {
        return "INSERT OR REPLACE INTO `video_model` (`kind`,`pageInfo`,`etag`,`items`) VALUES (?,?,?,?)";
      }

      @Override
      public void bind(SupportSQLiteStatement stmt, DetailVideoModel value) {
        if (value.getKind() == null) {
          stmt.bindNull(1);
        } else {
          stmt.bindString(1, value.getKind());
  ...
  @Override
  public Object insertDetailVideo(final DetailVideoModel item,
      final Continuation<? super Unit> p1) {
    return CoroutinesRoom.execute(__db, true, new Callable<Unit>() {
      @Override
      public Unit call() throws Exception {
        __db.beginTransaction();
        try {
          __insertionAdapterOfDetailVideoModel.insert(item);
          __db.setTransactionSuccessful();
          return Unit.INSTANCE;
        } finally {
          __db.endTransaction();
  ...
  @Override
  public Object fetchDetailVideo(final Continuation<? super List<DetailVideoModel>> p0) {
    final String _sql = "SELECT * FROM video_model";
    final RoomSQLiteQuery _statement = RoomSQLiteQuery.acquire(_sql, 0);
    return CoroutinesRoom.execute(__db, false, new Callable<List<DetailVideoModel>>() {
      @Override
      public List<DetailVideoModel> call() throws Exception {
        final Cursor _cursor = DBUtil.query(__db, _statement, false, null);
        try {
          final int _cursorIndexOfKind = CursorUtil.getColumnIndexOrThrow(_cursor, "kind");
          final int _cursorIndexOfPageInfo = CursorUtil.getColumnIndexOrThrow(_cursor, "pageInfo");
          final int _cursorIndexOfEtag = CursorUtil.getColumnIndexOrThrow(_cursor, "etag");
          final int _cursorIndexOfItems = CursorUtil.getColumnIndexOrThrow(_cursor, "items");
          final List<DetailVideoModel> _result = new ArrayList<DetailVideoModel>(_cursor.getCount());
          while(_cursor.moveToNext()) {
            final DetailVideoModel _item;
            final String _tmpKind;
            _tmpKind = _cursor.getString(_cursorIndexOfKind);
            final PageInfo _tmpPageInfo;
            final String _tmp;
            ...
            _tmp_1 = _cursor.getString(_cursorIndexOfItems);
            _tmpItems = __playlistModelTypeConverter.fromItemsItem(_tmp_1);
            _item = new DetailVideoModel(_tmpKind,_tmpPageInfo,_tmpEtag,_tmpItems);
            _result.add(_item);
          }
          return _result;
        } finally {
